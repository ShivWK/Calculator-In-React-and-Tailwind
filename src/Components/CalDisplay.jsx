...
export default function CalDisplay(props){
    ...
    return(
        <div className='w-full my-4 h-20 rounded-lg flex pt-2 px-2 gap-2 flex-col items-center justify-center' style={{backgroundColor: props.dispColor,
...
{/* for operator use unicode of them not english alphabets */}
            <input type="text" readOnly value={props.inputValue} className='border-none outline-none flex items-center rounded-md w-full text-4xl bg-transparent' style={{color: context.textColor, textAlign : 'end'
}}/>
        </div>
    )
}